...
@interface DVGAlertAction ()
@property (nonatomic, copy) NSString *title;
@property (nonatomic, assign) DVGAlertActionStyle style;
@property (nonatomic, copy) void (^handler)(DVGAlertAction *action);
- (void)performAction;
@end

@implementation DVGAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(DVGAlertActionStyle)style handler:(void (^)(DVGAlertAction *action))handler {
...
- (instancetype)initWithTitle:(NSString *)title style:(DVGAlertActionStyle)style handler:(void (^)(DVGAlertAction *action))handler {
    if ((self = [super init])) {
        _title = [title copy];
        _style = style;
...
@interface DVGAlertController () <UIActionSheetDelegate, UIAlertViewDelegate> {
    struct {
        unsigned int isShowingAlert:1;
    } _flags;
}
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(DVGAlertControllerStyle)preferredStyle NS_DESIGNATED_INITIALIZER;
...
// Universal
@property (nonatomic, weak) DVGAlertAction *executedAlertAction;
...
@implementation DVGAlertController
...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(DVGAlertControllerStyle)preferredStyle {
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(DVGAlertControllerStyle)preferredStyle {
    if ((self = [super init])) {
        _title = [title copy];
        ...
        if ([self alertControllerAvailable]) {
            ...
            if (preferredStyle == DVGAlertControllerStyleActionSheet) {
...
- (void)addAction:(DVGAlertAction *)action {
    NSAssert([action isKindOfClass:DVGAlertAction.class], @"Must be of type DVGAlertAction");
    ...
    if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == DVGAlertControllerStyleActionSheet) {
            NSUInteger currentButtonIndex = [self.actionSheet addButtonWithTitle:action.title];

            if (action.style == DVGAlertActionStyleDestructive) {
                self.actionSheet.destructiveButtonIndex = currentButtonIndex;
            } else if (action.style == DVGAlertActionStyleCancel) {
            ...
            if (action.style == DVGAlertActionStyleCancel) {
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField *textField))configurationHandler {
    if ([self alertControllerAvailable]) {
        ...
        NSAssert(self.preferredStyle == DVGAlertControllerStyleAlert, @"Text fields are only supported for alerts.");
...
- (NSArray *)textFields {
    ...
    } else if (self.preferredStyle == DVGAlertControllerStyleAlert) {
...
- (void)showWithSender:(id)sender arrowDirection:(UIPopoverArrowDirection)arrowDirection controller:(UIViewController *)controller animated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == DVGAlertControllerStyleAlert) {
            if (!controller) {
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    if ([self alertControllerAvailable]) {
        ...
        if (completion) {
            [self addDidDismissBlock:^(DVGAlertAction *action) { completion(); }];
        }
        if (self.preferredStyle == DVGAlertControllerStyleActionSheet) {
...
- (DVGAlertAction *)actionForButtonIndex:(NSInteger)index {
    return index >= 0 ? self.actions[index] : nil;
}

- (void)performBlocks:(NSString *)blocksStorageName withAction:(DVGAlertAction *)alertAction {
    // Load variable and nil out.
    NSArray *blocks = [self valueForKey:blocksStorageName];
    [self setValue:nil forKey:blocksStorageName];

    for (void (^block)(DVGAlertAction *action) in blocks) {
        block(alertAction);
    }
}

- (void)viewWillDismissWithButtonIndex:(NSInteger)buttonIndex {
    DVGAlertAction *action = [self actionForButtonIndex:buttonIndex];
...
+ (instancetype)actionSheetWithTitle:(NSString *)title {
    return [[self alloc] initWithTitle:title message:nil preferredStyle:DVGAlertControllerStyleActionSheet];
}
...
+ (instancetype)presentDismissableAlertWithTitle:(NSString *)title message:(NSString *)message controller:(UIViewController *)controller {
    DVGAlertController *alertController = [self alertWithTitle:title message:message];
    [alertController addAction:[DVGAlertAction actionWithTitle:NSLocalizedString(@"Dismiss", @"") style:DVGAlertActionStyleCancel handler:NULL]];
...
- (void)addCancelActionWithHandler:(void (^)(DVGAlertAction *action))handler {
    [self addAction:[DVGAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"") style:DVGAlertActionStyleCancel handler:handler]];
}
