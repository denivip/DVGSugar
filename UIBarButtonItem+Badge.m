...
NSString const *UIBarButtonItem_badgeBrdColorKey = @"UIBarButtonItem_badgeBrdColorKey";
...
@dynamic badgeValue, badgeBGColor, badgeBrdColor, badgeTextColor, badgeFont;
...
- (void)refreshBadge
{
    ...
    if(self.badgeBrdColor != nil){
        self.badge.layer.borderWidth = 3.0;
        self.badge.layer.borderColor = self.badgeBrdColor.CGColor;
    }
...
// Badge border color
-(UIColor *)badgeBrdColor {
    return objc_getAssociatedObject(self, &UIBarButtonItem_badgeBrdColorKey);
}
-(void)setBadgeBrdColor:(UIColor *)badgeBrdColor
{
    objc_setAssociatedObject(self, &UIBarButtonItem_badgeBrdColorKey, badgeBrdColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.badge) {
        [self refreshBadge];
    }
}
